package com.techupstudio.GeneralUtils;
 ...
 */
    ...
    public static Character toCharacter(Object obj) { return (char) obj.hashCode(); }
...
//        public abstract void onResume();
    ...
    public static Date datetime(){ return new Date(); }
    ...
    public static class SortedNumArray{
        ...
        private Double[] sort(){ return swapSort(Funcs.toDouble(DATA.toArray())); }
        ...
        public Integer[] toInteger(){ return Funcs.toInteger(sort()); }
        public Float[] toFloat(){ return Funcs.toFloat(sort()); }
    ...
    public static class MergeSort {
        ...
        private Integer[] mergeSort(Integer[] array, int start, int end) {
            if(end-start > 1) {
                ...
                if (end-start == 2) {
                    if (array[start] > array[end-1]) { swap(array, start, end-1); }
                }
                else{
        ...
        private Integer[] mergeArray(Integer[] array, int start, int middle, int end ){
            ...
            for (int i : range(start,end)){
                if (array[i] > array[middle]){ swap(new_array,i,middle); }
    ...
    public static String[] split(Object line, Object at){
        ...
        if (line.toString().contains("<>".replace("<>", at.toString()))){
            boolean match;
            StringBuilder ret = new StringBuilder();line = line.toString() + at.toString();
    ...
    public static String wrapLine(Object line, Integer wraplenght){
        ...
        for (int j=0;j<len(newlineIndex);j++){
            try {
                ...
                ret_Str.append(newline.repeat(newlineIndex.get(j))).append(wrapped);
            }catch (Exception e){
                ret_Str.append(newline.repeat(newlineIndex.get(j)));
            }
        }
    ...
    public static String alignLeft(Object line, Integer barlength){
        ...
        line = wrapLine(line, barlength);
        Map<Integer, Integer> newlineIndex = getNewlineIndex(line);
        StringBuilder ret_Str = new StringBuilder();String newline = "\n";String phrase;
        for (int j=0;j<len(newlineIndex);j++){
            try {
                phrase = split(line.toString().trim(), "\n")[j].trim();
                ret_Str.append(newline.repeat(newlineIndex.get(j))).append(phrase);
            }catch (Exception e){
                ret_Str.append(newline.repeat(newlineIndex.get(j)));
            }
        }
    ...
    public static String alignRight(Object line, Integer barlength){
        ...
        for (int j=0;j<len(newlineIndex);j++){
            try {
                String phrase = split(line.toString().trim(), "\n")[j].trim();
                ret_Str.append(newline.repeat(newlineIndex.get(j))).append(" ".repeat(barlength - phrase.length())).append(phrase);
            }catch (Exception e){
                ret_Str.append(newline.repeat(newlineIndex.get(j)));
            }
        }
        if (newlineIndex.isEmpty()){
            ...
            ret_Str.append(" ".repeat(barlength - phrase.length())).append(phrase);
        }
        return ret_Str.toString();
    }
    ...
    public static String replaceAll(String line, String section, String newString){
        while (line.contains(section)){ line = replaceFirst(line,section,newString); }
        return line;
    }
    ...
    public static String format(String line, Object... with){
        ...
        StringBuilder newline = new StringBuilder(line);Integer i = 1;
